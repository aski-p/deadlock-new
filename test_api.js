        ...
        if (response.data && response.data.length > 0) {
          ...
          if (firstPlayer && firstPlayer.player && firstPlayer.player.accountId) {
            ...
            try {
              ...
              const matchResponse = await axios.get(`https://api.deadlock-api.com/v1/players/${testAccountId}/match-history`, {
                timeout: 10000,
                headers: {
                  'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36'
                }
              ...
              if (matchResponse.data && Array.isArray(matchResponse.data)) {
                ...
                matchResponse.data.forEach((match) => {
                  ...
                  if (heroId === 11) {
                    infernusCount++;
                    console.log(`🔥 Infernus match found: Match ID ${match.match_id}, Hero ID ${heroId}`);
                  }
                ...
                console.log(`📈 Top heroes by ID:`, Object.entries(heroCount)
                  .sort((a, b) => b[1] - a[1])
                  .slice(0, 5)

...
async function testCompleteItemFlow() {
  ...
  const getItemImage = (itemName) => {
    return frontendItemImageMap[itemName] || 'https://cdn.deadlock.coach/vpk/panorama/images/items/weapon/basic_magazine.webp';
  };
  ...
  for (const item of selectedItems) {
    const imageUrl = getItemImage(item.name);
    if (imageUrl !== 'https://cdn.deadlock.coach/vpk/panorama/images/items/weapon/basic_magazine.webp') {
      try {
        const response = await axios.head(imageUrl, { timeout: 5000 });
        console.log(`✅ ${item.name}: ${response.status} ${response.statusText}`);
      } catch (error) {
        console.log(`❌ ${item.name}: ${error.response?.status || 'NETWORK_ERROR'} - ${error.message}`);
      }
    }
  }

...
const supabaseServiceKey = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImRwbW9hZmdheXNvY2ZqeGxtYXVtIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc1MTQ2NDMzMSwiZXhwIjoyMDY3MDQwMzMxfQ.G2woWTLhGpc0FOEyfABZs7k1wYTSYCaDeYhYtpoY73c';
...
async function createTables() {
  ...
  try {
    ...
    if (postsError) {
      ...
      const { data: testData, error: testError } = await supabase.from('board_posts').select('count').limit(1);
    ...
    if (commentsError) {
      console.log('💬 직접 테이블 확인 시도...');
      const { data: testData, error: testError } = await supabase.from('board_comments').select('count').limit(1);
      if (testError && testError.code === 'PGRST116') {
